...
func TestParseConfig_UsesGlobal(t *testing.T) {
	...
	if cfg.WebPort != 7777 || cfg.LogLvl != "error" { t.Fatalf("flag precedence wrong: %+v", cfg) }
...
func TestMain_ReturnCodes(t *testing.T) {
	...
	if code := Main(); code != 0 { t.Fatalf("help expected 0, got %d", code) }
	// empty
	os.Args = []string{"testctl"}
	if code := Main(); code != 2 { t.Fatalf("empty expected 2, got %d", code) }
	...
	if code := Main(); code != 0 { t.Fatalf("expected 0, got %d", code) }
}
