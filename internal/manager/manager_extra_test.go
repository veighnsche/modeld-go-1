...
func TestEnsureInstance_FastPathNoDoubleCount(t *testing.T) {
	...
	if err := m.EnsureInstance(context.Background(), "m"); err != nil {
		t.Fatalf("ensure fast: %v", err)
	}
	m.mu.RLock()
	inst2 := m.instances["m"]
	used2 := m.usedEstMB
	last2 := inst2.LastUsed
	m.mu.RUnlock()
	if used2 != used {
		t.Fatalf("usedEstMB changed on fast path: %d -> %d", used, used2)
	}
	if !last2.After(last) {
		t.Fatalf("LastUsed not updated on fast path")
	}
}

func TestEnsureModel_NoChangeWhenSame(t *testing.T) {
	m := NewWithConfig(ManagerConfig{})
	if err := m.EnsureModel(context.Background(), "m1"); err != nil {
		t.Fatalf("ensure model: %v", err)
	}
	if err := m.EnsureModel(context.Background(), "m1"); err != nil {
		t.Fatalf("ensure same: %v", err)
	}
	snap := m.Snapshot()
	if snap.State != StateReady || snap.CurrentModel == nil || snap.CurrentModel.ID != "m1" {
...
func TestEnsureModel_ContextCanceledSetsError(t *testing.T) {
	...
	cancel()
	if err := m.EnsureModel(ctx, "m2"); err == nil {
...
func TestEvictUntilFits_NoIdleInstances(t *testing.T) {
	...
	usedAfter := m.usedEstMB
	m.mu.RUnlock()
	if !hasA || !hasB {
		t.Fatalf("instances should remain when no idle")
	}
	if usedAfter != usedBefore {
		t.Fatalf("usedEstMB changed: %d -> %d", usedBefore, usedAfter)
	}
	// cleanup channels
	<-ia.genCh
	<-ia.queueCh
...
func TestGetModelByID(t *testing.T) {
	...
	if mdl, ok := m.getModelByID("b"); !ok || mdl.ID != "b" {
		t.Fatalf("expected to find model b, got %+v ok=%v", mdl, ok)
	}
	if _, ok := m.getModelByID("z"); ok {
		t.Fatalf("expected not found for z")
	}
}
