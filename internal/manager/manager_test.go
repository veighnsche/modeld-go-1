...
func createModelFile(t *testing.T, dir, name string, sizeMB int) string {
    ...
    for i := 0; i < sizeMB; i++ {
        if _, err := f.Write(block); err != nil { t.Fatalf("write: %v", err) }
    }
...
func TestReadyReflectsInstance(t *testing.T) {
    ...
    if err := m.EnsureInstance(ctx, "m1"); err != nil { t.Fatalf("ensure: %v", err) }
    if !m.Ready() {
...
func TestEvictionLRUUntilFits(t *testing.T) {
    ...
    if err := m.EnsureInstance(context.Background(), "c"); err != nil { t.Fatalf("ensure c: %v", err) }
